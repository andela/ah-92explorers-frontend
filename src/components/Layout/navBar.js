      ...
      <Fragment>
        <div className="navbar">
          ...
          <div className="restOfNav">
                  ...
                  <ListGroupItem
                    ...
                    data-value={
                      this.isHTML(notifs.message)
                        ? `${notifs.message.split('<')[0]} ${notifs.message.match(regex)[2].split('/').pop().split('-').join(' ')}`
                        : notifs.message
                    }
                    data-key={`${notifs.id}`}
                  >
                      ...
                      this.isHTML(notifs.message)
                        ? (
                          <span className="articleComment">
                            ...
                            <a className="articleComment" href={`/article/${notifs.message.match(regex)[2].split('/').pop()}`}>
                              <span className="articleLink">{`${notifs.message.match(regex)[2].split('/').pop().split('-').join(' ')}`}</span>
                            </a>
                          </span>
                        ) : notifs.message
                    }
                    {' '}
                    {notifs.status === 'read' ? '' : <span className="unreadStatus">{notifs.status}</span>}
                    <br></br>
            ...
            <img src={localStorage.getItem('image')} alt="" className="navIcons manIcon" />
            <Dropdown />
          </div>
        </div>
